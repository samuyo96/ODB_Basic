...
CREATE TABLE Pedidos (
    ...
    (START WITH 1 INCREMENT BY 1 NOCACHE) PRIMARY KEY,
    Cl_ID INT,
    FOREIGN KEY (Cl_ID) REFERENCES Clientes ON DELETE CASCADE,
    P_Cost DECIMAL(10,2),
...
--Procedimientos encapsultados D (Eliminar) de CRUD 
...
BEGIN
    ...
    IF Exist THEN
        ...
        WHERE Cl_ID = In_Cl_ID;
        DBMS_OUTPUT.PUT_LINE('Cliente con ID ' || In_Cl_ID || ' ha sido eliminado.');
    ELSE
        RAISE_APPLICATION_ERROR(-20002, 'El cliente con ID especificado no existe.');
    END IF;
END;
...
BEGIN
    ...
    IF Exist THEN
        DELETE FROM pedidos
        WHERE P_ID = In_P_ID;
        DBMS_OUTPUT.PUT_LINE('Pedido con ID ' || In_P_ID || ' ha sido eliminado.');
    ELSE
        RAISE_APPLICATION_ERROR(-20002, 'El pedido con ID especificado no existe.');
    END IF;
END;
